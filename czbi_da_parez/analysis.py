...
Functions:
    load_data, assign_columns, assign_type, assign_imageID, assign_source,
    group_and_pivot_median, group_and_pivot_mean, drop_columns, correlation_analysis,
    tsne_clustering, explain_tSne, absolute_scale_difference, umap_clustering,
    hdbscan_clustering, plot_umap_embedding, get_umap_feature_importance,
    pca_analysis, pca_visualization, variance_analysis, run_feature_analysis
"""
...
def get_umap_feature_importance(
    ...
    Args:
        ...
        umap_embedding (np.ndarray): The 2D (or 3D) UMAP embedding coordinates. Shape should be (n_samples, n_components).
        feature_columns (list): A list of strings, containing the names of the original columns in `original_df` that were used to generate the UMAP embedding. These will be used as predictors for the Random Forest.
        test_size (float): The proportion of the dataset to include in the test split.
        random_state (int): Controls the randomness of the data splitting and Random Forest.
        n_top_features (int): The number of top features to display for each UMAP dimension.

    Returns:
        dict: A dictionary containing pandas Series of feature importances for each UMAP dimension (e.g., 'UMAP1_importance', 'UMAP2_importance').
    """
...
def pca_visualization(pca_df, path_4_png, input_type):
    ...
    plt.title(f'PCA Projection of Data ({input_type}) Colored by Source')
    plt.legend()
    plt.grid(True)
...
def run_feature_analysis():
    ...
    pca_visualization(pca_df, path_4_png, input_type)
